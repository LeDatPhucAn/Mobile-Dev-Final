% !TeX program = xelatex
% Reliable Windows build: powershell -NoProfile -ExecutionPolicy Bypass -File docs/build-report.ps1
...
% Use XeLaTeX, LuaLaTeX, or Tectonic for the Unicode text.
...
\textbf{Keywords:} Android; on-device inference; image retrieval; semantic search; face recognition; OCR; local persistence.


\subsection*{Report Organization}
Sections 1--5 establish the application context, work allocation, requirements, and screens. Sections 6--13 describe usability, architecture, implementation, and technology. Sections 14--15 document setup and installation. Sections 16--18 present validation, self-assessment, and limitations.
...
Text-based image retrieval & OCR search retrieves photos containing specified words or numbers.\\
...
The primary target group comprises Android users managing heterogeneous personal image collections who require content-based retrieval without comprehensive manual tagging. English visual descriptions remain the most suitable input for Normal query.
...
Student & Recover a saved receipt or message screenshot without remembering its date. & OCR search \(\rightarrow\) enter “invoice” \(\rightarrow\) open result \(\rightarrow\) Read text.\\
...
\begin{enumerate}
\item \textbf{Recover a document image.} The user selects OCR search and enters words actually visible in a screenshot. The application returns matching photos and preserves the original recognized text in the reader. The user copies the required passage.
...
Requirements and product scope & Tran Canh Anh Tuan & Requirement mapping, topic, use cases, and demonstration sequence in COURSE\_REQUIREMENTS.md and README. Documentation exists.\\
...
OCR and text retrieval & Le Dat Phuc An & Bundled recognition, FTS4 search, text reader. Implemented in OCR/data/UI components.\\
Background indexing & Le Dat Phuc An & Incremental worker, scheduling, and consistency protection. Implemented in worker/ and repositories.\\
...
Text reader & View, select, and copy the original recognized text.\\
\end{reporttable}
...
3 \(\rightarrow\) 4 & Add original/normalized OCR text, an FTS4 index and synchronization triggers, plus history search-mode storage.\\
\end{reporttable}
...
\reportsection{OCR and Text Retrieval}
...
The index normalizes recognized text for word matching, including lowercasing text and turning punctuation into word separators. Original text remains available for display and copying. User input is converted to quoted literal terms for the FTS4 query; it is not treated as executable FTS syntax. A query permits at most 64 distinct terms.
...
\code{invoice total} & Match the words “invoice” and “total.” & All terms must match.\\
...
\code{@alex invoice} & Resolve the saved identity and require the OCR terms. & Requires face indexing and an enrolled matching handle.\\
...
Date sorting is applied while selecting OCR matches, giving the true oldest/newest 100 across all matching pages. The reader displays the original recognized text and supports selection/copying. Sharp printed text and screenshots are suitable inputs; tiny text, blur, handwriting, and poor contrast can lower recognition quality.
...
ML Kit text recognition & 16.0.1 & Bundled offline Latin text recognition.\\
...
\begin{enumerate}
\item Grant photo access. Android 10--12L uses \path{READ_EXTERNAL_STORAGE}; Android 13+ uses \path{READ_MEDIA_IMAGES}. Android 14+ also supports selected-photo access through \path{READ_MEDIA_VISUAL_USER_SELECTED}.
\item Start with a small accessible set containing a clear scene, a single-face portrait, and a legible screenshot or receipt. Open Albums to confirm actual device photos are visible.
...
Unit tests cover vector math, embedding output validation, exact top-K ranking, deterministic ties, OCR date ordering across pages, mention parsing, distinct face assignment, text normalization, date bounds, and incremental planning. Instrumentation covers Room migrations/cascades, revision protection, OCR search on metadata-only photo rows, real bundled OCR/CLIP/face inference, copy success/failure cleanup, filter restoration, and landscape viewer actions.
...
After completing Section~\ref{sec:setup}, run the following tasks with the wrapper on a dedicated test emulator:
\begin{verbatim}
:app:testDebugUnitTest
:app:lintDebug
:app:connectedDebugAndroidTest
\end{verbatim}
The project's \path{android.injected.androidTest.leaveApksInstalledAfterRun=true} setting retains installed APKs after connected tests to reduce unintended loss of a development index.
...
Originality and complexity & 15\% & Integration of offline cross-modal retrieval, distinct-person matching, OCR/FTS, persistence, and Android media operations using pretrained models. Comparative originality and suitability for the confirmed group size remain subject to assessor review.\\
...
Normal-query visual descriptions work best in English. OCR can miss small, blurred, handwritten, or poorly contrasted text. Face detection can miss small or obscured faces, and uncalibrated identity matching can produce false matches.
